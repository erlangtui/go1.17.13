...
// memmove 从 from 拷贝 n 个字节到 to
// memmove确保 from 中的任何指针都以不可分割的写方式写入 to ，
// 因此，racy 读取无法观察到写入一半的指针。这是防止垃圾回收器观察到无效指针所必需的，与非托管语言中的 memmove 不同。
// 然而，只有当 from 和 to 可能包含指针时才需要使用memmove，只有当“from”、“to”和“n”都对齐单词时才会出现这种情况。
// memmove 确保 from 中的任何指针都以不可分割的写入 to，
// 具体实现在 memmove_*.s 汇编文件
//go:noescape
func memmove(to, from unsafe.Pointer, n uintptr)

// 在程序集之外调用memmove。确保它有ABI包装器。
//go:linkname memmove
